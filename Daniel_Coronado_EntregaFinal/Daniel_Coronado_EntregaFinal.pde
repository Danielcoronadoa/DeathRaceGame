...
for(int i = 0; i < numBombas; i++){
 ...
 bombas[i].rebotar();
   ...
   newRow.setInt("id", Puntajes.lastRowIndex());
   newRow.setInt("Puntaje",Ranking);
   newRow.setString("name", "Lion");
      } 
    saveTable(Puntajes,"data/Podio.csv");
   //Tiempo
   Ranking = (TiempoJuego-TiempoInicial/60)*-1;
   
 fill(200,0,0);
 textSize(21);
 text( "TIEMPO:" + Ranking, 0, 30); 
  ...
  //Tiempo total jugado
   TiempoJuego = millis()-TiempoInicial; 
 } 
}
void keyReleased(){
  if (key=='1'){
